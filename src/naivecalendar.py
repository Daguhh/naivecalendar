...
def ask_theme():
    ...
    #themes = list(chain(*[glob.glob(f'{path}/common/theme*.rasi') for path in THEME_PATHS.values()]))
    themes = list(glob.glob(f"{THEME_PATHS['rel']}/common/theme*.rasi"))
    themes = (t.split('/')[-1].split('.')[0]for t in themes)
    themes = list2rofi(t for t in sorted(set(themes)) if t!="config")

    theme = rofi_popup("Choose a color theme", themes)
    if not theme:
        return

    #shapes = list(chain(*[glob.glob(f'{path}/common/shape*.rasi') for path in THEME_PATHS.values()]))
    shapes = list(glob.glob(f"{THEME_PATHS['rel']}/common/shape*.rasi"))
    shapes = (t.split('/')[-1].split('.')[0]for t in shapes)
    shapes = list2rofi(t for t in sorted(set(shapes)) if t!="config")

    shape = rofi_popup("Choose a shape", shapes)
    if not shape:
        return
...
@import "{THEME_PATHS['rel']}/common/{theme}.rasi"
@import "{THEME_PATHS['rel']}/common/position.rasi"
@import "{THEME_PATHS['rel']}/common/{shape}.rasi"
"""

    first_time_init()

    with open(f"{THEME_PATHS['user']}/{DEFAULT_THEME}.rasi", 'w') as conf_file:
        conf_file.write(theme_config)

    if ( THEME_PATHS['user'] / "config.cfg" ).exists():
        os.remove(THEME_PATHS['user'] / f"{DEFAULT_THEME}.cfg")

    shutil.copy(
        f"{THEME_PATHS['rel']}/common/{shape}.cfg",
        f"{THEME_PATHS['user']}/{DEFAULT_THEME}.cfg"
    )
